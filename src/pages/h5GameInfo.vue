<template>
    <div class="app">
         ...
         <scroll-view class="scrollList">
                 <div class="gameTop">
                     <image class="glogo" :src="detailInfo.logo | handleUrl"></image>
                     <div class="info">
                         <div>
                              <text class="gt">{{detailInfo.name}}</text>
                              <text class="gstyle">{{detailInfo.typeName}}</text>
                         </div>
                        <div>
                            <button class="downBtn azdown" :eeui="{text:'进入', borderWidth:1,borderColor:'#ffbd58',backgroundColor:'#fff'}" @click="gameLinkH5()"></button>
                        </div>
                     </div>
                 </div>
                <div class="gameDetail">
                    ...
                    <div class="gameInfo" v-if="position==0">
                        <div class="slidePad" v-if="picListsss.length">
                            <wxc-ep-slider :slider-id="sliderId"
                                        :card-length='picListsss.length'
                                        :card-s="cardSize"
                                        :select-index="1"
                                        @wxcEpSliderCurrentIndexSelected="wxcEpSliderCurrentIndexSelected">

                                <div v-for="(v,index) in picListsss"
                                    :key="index"
                                    :slot="`card${index}_${sliderId}`"
                                    :class="['slider',`slider${index}`]">
                                    <image class="gamePic"  :src="v | handleUrl"></image>
                                </div>
                            </wxc-ep-slider> 
                        </div>
                        <div class="desInfo">
                    ...
                    <div class="giftBCon" v-if="position==1">
                        ...
                        <div>
                            <div class="giftBox" :key="index" v-for="(item,index) in giftDetailList.gifts" >
                                <div class="giftLf">
                                    <text class="giftname">{{item.name}}</text>
                                    <richtext class="giftC">
                                        <span>礼包内容：</span>
                                        <span :key="k" v-for="(v,k) in item.content">{{v.title+'、'}}</span>
                                    </richtext>
                                </div>
                                <button class="receiveB" :eeui="{text:'领取', borderWidth:2,borderColor:'#ffbd58',backgroundColor:'#fff'}" @click="recGift(item)"></button>
                            </div>
                        </div>
                    </div>
